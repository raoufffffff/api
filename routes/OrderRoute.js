        ...
        const ServerCancel = async () => {
            try {
                for (let i = 0; i < a.length; i++) {
                    ...
                    if (a[i].IsLivrorAccepts) {
                        return
                    }
                    await Order.findOneAndUpdate(a[i]._id, a[i])
                }
            } catch (error) {
                console.log('tnakhet');
            }
...
OrderRoute.put('/livror/:id', async (req, res) => {
    ...
    try {
        ...
        if (livror.haveorder?.workin) {
            return res.send({ "success": false, 'message': 'you have an order ' })
        }
...
// livror cancel becose user not res
OrderRoute.put('/usernot/res/:id', async (req, res) => {
    const { id } = req.params
    const livrorid = req.body.livrorid
    try {
        let order = await Order.findById(id)
        ...
        if (order.livrorId == livror._id) {
            ...
            // 
            await Livror.findByIdAndUpdate(livrorid, livror)
